...
export class MetadataAccumulator {
  ...
  parseAnnotation(testId: string, annotation: TestAnnotation): void {
    ...
    if (annotation.message.startsWith('Tiden Profiler Steps: ')) {
      try {
        const stepsJson = annotation.message.replace('Tiden Profiler Steps: ', '');
    ...
    if (!annotation.message.startsWith('Tiden ')) {
      // Unknown non-Tiden message — but we already created the entry. Remove it
      // to preserve "non-Tiden annotation is ignored" semantics for the very
      // first annotation if no Tiden data was added.
      if (
    ...
    const tidenType = annotation.message.split(':')[0]?.replace('Tiden ', '').toLowerCase().replace(' ', '-') ?? '';

    switch (tidenType) {
      case 'title':
        metadata.title = annotation.message.replace('Tiden Title: ', '');
        break;

      case 'comment':
        metadata.comment = annotation.message.replace('Tiden Comment: ', '');
        break;

      case 'suite':
        metadata.suite = annotation.message.replace('Tiden Suite: ', '');
        break;

      case 'fields': {
        const fieldsData = annotation.message.replace('Tiden Fields: ', '');
        try {
      ...
      case 'parameters': {
        const parametersData = annotation.message.replace('Tiden Parameters: ', '');
        try {
      ...
      case 'group-parameters': {
        const groupParametersData = annotation.message.replace('Tiden Group Parameters: ', '');
        try {
      ...
      case 'tags': {
        const tagsData = annotation.message.replace('Tiden Tags: ', '');
        try {
      ...
      case 'step-start':
        metadata.currentStep = annotation.message.replace('Tiden Step Start: ', '');
        break;
      ...
      case 'attachment': {
        const attachmentName = annotation.message.replace('Tiden Attachment: ', '');
