...
import { ConfigLoader, composeOptions } from '@tiden/reporter-commons';
 ...
 * cross-worker communication channel used by all other Tiden metadata.
 *
 * Usage: set `setupFiles: ['@tiden/vitest-reporter/setup']` in vitest.config.ts
 *
 * Limitation: sequential tests within a worker only. test.concurrent() or
 * parallel test execution will produce incorrect step attribution.
 */
...
if (options.profilers?.includes('network')) {
  ...
  afterEach((context) => {
    ...
    if (newSteps.length > 0) {
      ...
      (context.task.meta as any)._tidenProfilerSteps = JSON.stringify(newSteps);
    }
  });
}
